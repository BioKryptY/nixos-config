...
keymap.set("n", "N", "Nzzzv",
           {desc = "Go to the previous search result and center"})
...
keymap.set("n", "<Leader>te", ':tabedit <C-r>=expand("%:p:h")<CR>/',
           {desc = "Open new tab with current buffer's path"})

-- Indent or de-indent
keymap.set("n", "<Tab>",
           function() return require('utils').always_working_indent_line() end,
...
keymap.set("n", "<C-f>", function()
    if vim.api.nvim_buf_get_option(vim.api.nvim_win_get_buf(0), "filetype") ==
        "oil" then
...
if vim.fn.has("nvim-0.8") == 1 then
    keymap.set("n", "<Leader>pp", function() return vim.lsp.buf.format() end,
               {desc = "Fix document formatting"})
else
    keymap.set("n", "<Leader>pp",
               function() return vim.lsp.buf.formatting() end,
               {desc = "Fix document formatting"})
end

keymap.set("v", "<Leader>pp",
           function() return vim.lsp.buf.range_formatting() end,
           {desc = "Fix document formatting"})
...
-- b64.nvim
keymap.set("v", "<leader>be", ":<C-u>lua require('b64').encode()<CR>",
           {desc = "Encode text with base64"})
keymap.set("v", "<leader>bd", ":<C-u>lua require('b64').decode()<CR>",
